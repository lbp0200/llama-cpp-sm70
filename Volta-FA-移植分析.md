...
> [!NOTE]
> **历史文档（2026-09-21 撰写）。** 它提出的方案最终被证伪：移植后的内核在两条架构上都比上游
> 慢（V100 pp2048 -3.4% / pp16384 -20.9% / pp65536 -37.1%），代码已于 2026-09-26 删除。
> 本文保留作为推理链的起点与史料；结论请看 `Volta-Turing-FA进度.md`（尤其「V100 证据复核」
> 与「项目结论」两节）。下文中的收益估算（+15~33%）基于两个后来被推翻的数字：
> 1Cat 的「60.8 TFLOP/s」是 full-square logical 口径（同项目台账的 Causal 口径是 38.0），
> 以及「attention 占 prefill 63.7%」是从一个单参数拟合里反推出来的，不是实测。
