...
> [!IMPORTANT]
> **2026-09-26 终结：移植的代码已删除。** `fattn-v100.cuh`（1404 行）+ `fattn.cu` 的 9 行闸门
> + `probe5/6.cu` 已移除，分支现在只是纯 TurboQuant fork。原因是实测：该内核在两条架构上都比上游
> `fattn-mma-f16.cuh` 慢（V100 pp2048 -3.4% / pp16384 -20.9% / pp65536 -37.1%；2070 -8~-15%），
> 而且带着一个潜伏的 sm_70 编译断裂（route B 的 `ldmatrix`）。**本文下方的历史与证据全部保留**，
> 包括为什么这条路过不去。另：V100 现在**在线**（192.168.7.3）。
> V100 上真正有效的优化在 `v100-优化存档/`（`-b 2048 -ub 2048 --spec-type draft-mtp`）。
>
...
> 机器：V100 = 192.168.7.3（sm_70, 32G）；2.16 = 10.1.2.16 RTX 2070（sm_75，在线，`bolt-remote`）
...
- V100 路径：sm_70 全绿（4x470/470、sweep 7747）；但 A/B 证据**不支持**把它做成默认：
  pp512/pp2048/decode 与旧路径打平（±2%），**pp65536 落后 29~38%**（详见文末「V100 证据复核」）。
...
### 实测结论：候选 #1 中性（已回滚），瓶颈重新定性（2026-09-24）

| 改动 | pp4096 | 判定 |
|---|---|---|
| comb 扫描 shfl 化 | 2115.6 -> **2171.2**（+2.6%，σ5.4）| 入库 `8014c118e` |
| publish 16 lane x 1 趟（本 candidate #1）| 2171.2 -> 2173.8（+0.1%，**σ5.9 内中性**）| **回滚**（无实测增益，不留 diff）|

门禁与冒烟均绿（`ab-2070.sh` 回合：470/470 x2、7747/7747、smoke "Hello world."），所以这不是正确性问题，
而是**该段根本不在关键路径上**。

**重新定性**：softmax 区连吃两次「看起来必然有效」的优化却几乎不动 => 瓶颈不在 ALU 侧。最大嫌疑改为
**占用率/延迟隐藏**：`launch_bounds(256,1)` + smem ~50.6KB（>32KB）=> 每 SM 只能常驻 1 个 block
= **8 warp/SM = 12.5% 占用**，显存与 smem 延迟几乎没有别的 warp 可遮盖；对照 wmma pair 路径同样 256 线程
却更快，说明差距来自每 warp 的工作量与访存模式，而不仅是占用率。

**下一根杠杆（改为结构性，不再做 ALU 微优化）**：把每 block 线程数提到 512（16 warp），在 **BN64 内
把 sub 切到 8 份、每份 8 列**（Turing 原生 mma 就是 m16n8k16，n8 是天然粒度）——每 warp 工作量减半、
warp 数翻倍，smem 不变（仍 1 block/SM，但 warp 数从 8 到 16），延迟隐藏翻倍。这是「列内 warp 组织」的
最小可行版本；若它也不动，则要承认 -20.5% 的差距需要 split-KV + combine（旧路径架构），单独立项。

注意：`launch_bounds(256,2)` 无效（2 x 50.6KB > 64KB），已从待办中删去。

### 决定性测量：route B 引擎是寄存器受限（2026-09-24，`cuobjdump -res-usage`）

| 变体 | REG | STACK | 线程 | 备注 |
|---|---|---|---|---|
| cfg<32,64,512,0,0,0>（V100 wmma） | 64 | 192 | 512 | O 在 smem，故 512 线程可行 |
| cfg<16,64,256,0,0,0>（SM75 solo） | 114 | 128 | 256 | |
| cfg<32,48,256,1,1,0>（路线 A pair，2731） | 128 | 128 | 256 | 基线 |
| **cfg<32,64,256,1,1,1>（route B 引擎）** | **207** | **624** | 256 | O 常驻寄存器 128 个/线程是大头 |

复现：`/usr/local/cuda/bin/cuobjdump -res-usage build/bin/libggml-cuda.so`（注意 cuobjdump 不在非登录
shell 的 PATH 里，要用绝对路径；grep `flash_attn_ext_v100_kernel` 看模板实例）。

**由此否决两条待办**：

- **512 线程 / 16 warp（上一轮提的「下一根杠杆」）不可行**：512 线程时寄存器文件只允许 128 regs/线程，
  而引擎光 O 就要 128 -> 必然大规模 spill。V100 那个 512 线程变体只有 64 regs，正是因为它把 O 放 smem；
  而我们的 smem 已被 KV union(32KB) + O(32KB) 顶满，两边都放不下。**撤回该提议**。
- **`launch_bounds(256,2)`**：同因（smem > 32KB）无效。

**重新定性**：207/255 寄存器 + 624B 栈 => 内核处于寄存器压力上限，编译器为凑合寄存器会拉长依赖链、
并以 local memory 兜底；加上 1 block/SM（smem 50.6KB）只有 8 warp/SM 可遮盖延迟。这解释了
「三次 ALU 侧改动全部近中性」：瓶颈是寄存器/占用率的架构性上限，不是某段代码。

**到此的性能纪律结论**：当前引擎（pp4096 2171.2，差 wmma pair -20.5%）已接近该设计的架构上限；
...
## V100 证据复核（2026-09-24）：`GGML_V100_FA` 对 V100 到底优化了什么

起因：核对「fork FA 对 V100 有什么优化、依据是什么」。**复核发现此前若干处表述把单次异常值当成了结论**，
以下是档案中有据可查的原始测量（Qwen3.8-27B IQ4_XS, f16 KV, 同场 back-to-back）。

### 同场 A/B：我们内核 vs 旧路径

| 指标 | 我们内核 | 旧路径 `GGML_V100_FA=0` | 差距 | 出处 |
|---|---|---|---|---|
| pp512 | 784.3 | 781.3 | 打平 | 三方 A/B 表 |
| pp2048 | 774.6 / 774.7 | 757.5 / 783.4 | +2.3% / -1.1%（噪声内）| 真模型 A/B + 三方 A/B |
| pp65536 | **288.9**（v2 mask-trunc HEAD）/ 286.6 | **468.4 / 461.8** | **-38%** | 同上两表 |
| tg32 | 36.96（3 次）| 37.42（3 次）| **-1.2%，打平** | tg32 基线复测 |

### 唯一有量化正收益的 V100 优化

1. **KV bank-skew pad**（V100 类 +16 halfs，`78b23d5bf`）：pp65536 **298.4 -> 333.9（+11.9%）**；但该收益属于
   更早的配置，后续 v2 mask-trunc / BM32-BN64-smem-O 配置在长上下文回落到 **288.9**。
2. **cfg 拆分 + smem 预算分级**（V100 = BM32/BN64/512 线程，Turing = 256 线程）：支撑性改动，无独立性能声明。

其余 sm_70 相关改动均为**正确性**性质（Turing fragment 映射、mask 推导 KV 扫描上界 6716ea4a9、max-over-rows、
graph 空指针 491ecdd04），不是提速项。

### 必须作废的说法

- 早前「V100 decode +20%」源自旧路径单次 25.87±3.48 的**异常值**，档案里已自行作废；复测为 **-1.2%（打平）**。
- 早前头部摘要写的「pp65536 333」是**更早配置的最好值**，不是最终 HEAD 的值（最终 288.9，-38%）。

### 结论

**对 V100，fork 内核没有实测净优势**（打平或更差）。且档案已记录：旧路径长上下文 1.6x 优势的
**真因至今未定位**（遗留假说 = 旧 mma 的 `nbatch_fa` 细粒度 CTA 分段 + cp.async 多级流水）——
即移植的原始目标（V100 快 128K prefill）**在证据上未达成**。
...
**状态：停止（用户决定）。分支 `feature/v100-fa-port` 冻结，不再继续。**

### 净产出

| 项 | 结果 |
|---|---|
| 速度 | 对上游**两条架构都没有净优势**：2070 pp4096 -10.9% / pp8192 -13.2%；V100 pp512/2048/decode 打平、pp65536 -38% |
| 原始目标（V100 快 128K prefill）| **未达成**；旧路径长上下文优势的真因未定位（cp.async 假说已证不可能，见上）|
| 实际交付 | **2070 默认回退上游内核**（约 +12~15% prefill，即「消除自身回归」）；`GGML_V100_FA` 成为纯 opt-in 研究开关 |
| 知识资产 | 谱系澄清（1Cat dataflow vs 上游架构）、寄存器受限定性（207 regs/624B 栈）、A/B/C smem 预算档、probe5/probe6 探针、10+ 份存档日志、完整踩坑记录 |

### 关键教训（下次别重复）

1. **先建立同卡上游基线，再谈优化**。本季最大的方法论失误：整场 SM75 优化都在拿 fork 内核跟自己比，
   直到最后才发现上游内核从一开始就快 20%+。
2. **先量资源天花板**（`cuobjdump -res-usage`）再调优：207 regs / 1 block/SM 一量出来，微优化路线当天就该否。
3. **移植前先验证前提**：`fattn-v100.cuh` 头部引用的 llama.cpp #28037 已经说过「上游配置调优近最优、
   剩余空间在架构性方向（split-D / fp32 smem / cp.async）」，而 split-D 恰好是**没有移植**的那部分。

### 若要清理

**已于 2026-09-26 执行：** `fattn-v100.cuh`（1404 行）+ `fattn.cu` 那 9 行闸门已删除，
`probe5.cu` / `probe6.cu` 也随主体删除（它们只是 route B 引擎的验证探针）。`probe4.cu` 保留
（记录的是「fragment 展开是架构相关的」这个通用教训）。分支现已是纯 TurboQuant fork。

**必须保留**的是同分支里两个真 bug 修复：`fattn-vec.cuh` 的 D512 VEC sm_70/75 编译 stub、
`ggml-cuda.cu` 的 CUDA graph 空指针保护。

删除的理由：内核虽然**正确**（两架构都 470/470）但**更慢**（V100 -3~-37%，2070 -8~-15%），
而且它带着一个潜伏的 sm_70 编译断裂（route B 的 `ldmatrix`，见上文）。它的价值已转移到文档：
「为什么这条路过不去」的答案（上游赢在 KV 轴分段 + Q_in_reg，不在数据流形状）在本文与
`v100-优化存档/` 里完整保留，代码在 git 历史里可取回。
