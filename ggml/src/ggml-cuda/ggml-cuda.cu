...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        // [TAG_MUL_MAT_ID_CUDA_GRAPHS]
        // Note: src[0] is NULL for source-less ops (ARANGE/FILL/DIAG/...), so guard the TQ check.
        const bool is_tq_w = node->src[0] &&
            (node->src[0]->type == GGML_TYPE_TQ4_1S || node->src[0]->type == GGML_TYPE_TQ3_1S);
